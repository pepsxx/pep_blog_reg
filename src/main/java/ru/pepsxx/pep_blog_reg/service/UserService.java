...
public class UserService {
    ...
    public UserDto findUserById(IdModel idModel) {
        return userRepository.findById(idModel.getId())
                .map(userMapper::userUserToUserDto)
                .orElseThrow(() -> new UserNoteFound("Пользователь с id: %s не найден".formatted(idModel.getId())));
    }
}
